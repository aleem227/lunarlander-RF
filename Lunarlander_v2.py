...
def create_gif(frames, filename):
    imageio.mimsave(filename, frames, duration=1/30)  # Adjust the duration as needed

agent = Agent(state_size=8, action_size=4, seed=0)

os.environ['SDL_AUDIODRIVER'] = 'dummy'  # Disable audio

with SmartDisplay(visible=0, size=(1400, 900)) as disp:
    env = gym.make('LunarLander-v2')
    ...
    env.close()
    create_gif(frames, 'lunarlander.gif')
